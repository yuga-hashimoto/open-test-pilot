...
Verified evidence for the current workspace is 327 passing tests (3 skipped), a fresh Chromium run against `examples/fixtures/web/server.mjs`, a passed local HTML report under `.testpilot/runs/`, and a real Docker Runner → server upload with 8 artifacts. Live PostgreSQL, Redis, S3, Android Appium, iOS Appium/XCUITest, GitHub App repository write, and GitHub OAuth/session/organization flows have also been exercised. Production credentials and registry publication remain deployment-specific.
