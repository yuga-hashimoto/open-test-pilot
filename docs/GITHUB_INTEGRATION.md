...
The local Server exposes `GET /auth/github/start` and `GET /auth/github/callback`. It stores one-time OAuth state, resolves the GitHub user, and returns a short-lived OpenTestPilot session token after a successful code exchange; it never returns the GitHub access token to the browser response. Set `AUTH_REQUIRED=true` in hosted mode to require `Authorization: Bearer <sessionToken>` plus organization membership on `/v1` routes. Local development can keep the explicit `x-organization-id` tenant header path.
