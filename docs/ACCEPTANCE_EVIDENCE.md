...
| GitHub integration UI/API gate | A fresh server on port `3004` loaded repository `yuga-hashimoto/open-test-pilot` with installation `146977164` into the live Web GitHub view. The browser created tenant change request `change-request-02e975a6-92a9-4139-8a9c-f47a4d7d5313`. Draft PR and repository sync requests reached the new routes and returned explicit `503` configuration gates (`GitHub App pull requests are not configured`, `GitHub App sync is not configured`) rather than a simulated success. | Passed for tenant-safe UI/API routing and truthful credential gating; live GitHub writes still require the private key. |
