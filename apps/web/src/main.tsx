...
function RunsView({ runs, selectedRun, onSelect, api }: { runs: Run[]; selectedRun: Run | undefined; onSelect: (run: Run) => void; api: TestPilotApi | undefined }) { const [evidence, setEvidence] = useState<{ failures: Array<Record<string, unknown>>; artifacts: Array<{ id: string; key: string; size: number }>; report?: { status: string } }>(); useEffect(() => { if (api === undefined || selectedRun === undefined) { setEvidence(undefined); return; } void api.getRunEvidence(selectedRun.id).then((value) => setEvidence(value as unknown as typeof evidence)).catch(() => setEvidence(undefined)); }, [api, selectedRun]); return <div className="runs-layout"><section className="panel live-list"><div className="panel-header"><div><h2>Runs</h2><p>Execution records from the selected organization</p></div></div><div className="run-table"><div className="table-head"><span>TEST</span><span>BRANCH</span><span>DURATION</span><span>STATUS</span><span>WHEN</span></div>{runs.map((run) => <button className={`run-row ${selectedRun?.id === run.id ? 'selected' : ''}`} key={run.id} onClick={() => onSelect(run)}><span className="test-name"><span className={`run-icon ${run.status}`}><Icon name={run.status === 'passed' ? 'check' : run.status === 'failed' ? 'close' : 'play'} /></span><b>{run.test}</b></span><span className="branch"><Icon name="branch" />{run.branch}</span><span className="muted">{run.duration}</span><StatusPill status={run.status} /><span className="muted">{run.time}</span></button>)}</div></section><section className="panel live-list"><div className="panel-header"><div><h2>Evidence</h2><p>{selectedRun?.id ?? 'Select a run'}</p></div></div>{evidence === undefined ? <div className="empty-state compact"><p>Connect to the team server to load failures, artifacts, and report status.</p></div> : <div className="live-list-body"><div className="live-list-row"><div><b>Report status</b><span>{evidence.report?.status ?? 'unknown'}</span></div></div><div className="live-list-row"><div><b>Failures</b><span>{evidence.failures.length} structured failure records</span></div></div><div className="live-list-row"><div><b>Artifacts</b><span>{evidence.artifacts.length} uploaded artifacts</span></div></div>{evidence.failures.map((failure, index) => <pre className="manifest-code" key={index}>{JSON.stringify(failure, null, 2)}</pre>)}</div>}</section></div>; }
