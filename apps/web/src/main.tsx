...
function TestsView({ tests, onRun, live, editingTest, manifestText, manifestBaseline, manifestStatus, manifestLoading, onEdit, onManifestChange, onSave }: { tests: ApiTest[]; onRun: (test: ApiTest) => void; live: boolean; editingTest: ApiTest | undefined; manifestText: string; manifestBaseline: string; manifestStatus: string | undefined; manifestLoading: boolean; onEdit: (test: ApiTest) => void; onManifestChange: (value: string) => void; onSave: () => void }) {
  const [view, setView] = useState<'tree' | 'yaml' | 'generated' | 'graph' | 'diff'>('yaml');
  ...
  return <div className="tests-layout"><section className="panel live-list"><div className="panel-header"><div><h2>Tests</h2><p>{live ? 'Tests loaded from the selected organization' : 'Connect a team server to load tests'}</p></div></div>{tests.length === 0 ? <div className="empty-state compact"><div className="empty-icon"><Icon name="layers" /></div><p>No tests are registered for this organization yet.</p></div> : <div className="live-list-body">{tests.map((test) => <div className="live-list-row" key={test.id}><div><b>{test.name}</b><span>{test.manifestId} · {test.id}</span></div><div className="row-actions"><button className="text-button" onClick={() => onEdit(test)}>Edit</button><button className="text-button" onClick={() => onRun(test)}>Run →</button></div></div>)}</div>}</section>{editingTest !== undefined && <section className="panel manifest-editor"><div className="panel-header"><div><h2>{editingTest.name}</h2><p>Source-first Manifest editor · {manifestStatus ?? (manifestLoading ? 'loading…' : 'unsaved changes are local')}</p></div><button className="run-button" onClick={onSave} disabled={!live || manifestLoading}>Save</button></div><div className="editor-view-tabs">{(['tree', 'yaml', 'generated', 'graph', 'diff'] as const).map((item) => <button key={item} className={view === item ? 'selected' : ''} onClick={() => setView(item)}>{item === 'tree' ? 'Tree' : item === 'yaml' ? 'YAML' : item === 'generated' ? 'Generated TS' : item === 'graph' ? 'Graph' : 'Diff'}</button>)}</div>{view === 'tree' ? <div className="manifest-tree" aria-label="Manifest tree">{parsed === undefined ? <span className="editor-error">YAML is invalid</span> : <Tree value={parsed} />}</div> : view === 'yaml' ? <div className="monaco-editor-shell" aria-label="Manifest YAML"><Editor height="520px" defaultLanguage="yaml" theme="vs-dark" value={manifestText} onChange={(value) => onManifestChange(value ?? '')} options={{ minimap: { enabled: false }, wordWrap: 'on', readOnly: manifestLoading, automaticLayout: true, fontSize: 13 }} /></div> : view === 'graph' ? <ManifestGraph manifest={parsed} /> : <pre className="manifest-code" aria-label={view === 'generated' ? 'Generated TypeScript' : 'Manifest diff'}>{view === 'generated' ? generated : manifestDiff(manifestBaseline, manifestText)}</pre>}</section>}</div>;
}
