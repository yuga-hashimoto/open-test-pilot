...
import { createApi, getApiConfig, type ApiRun, type ApiSchedule, type ApiTest, type TestPilotApi } from './api.js';
import './style.css';
...
function App() {
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      </> : active === 'Tests' ? <TestsView tests={tests} onRun={startRun} live={api !== undefined} />
...
function TestsView({ tests, onRun, live }: { tests: ApiTest[]; onRun: (test: ApiTest) => void; live: boolean }) { return <section className="panel live-list"><div className="panel-header"><div><h2>Tests</h2><p>{live ? 'Tests loaded from the selected organization' : 'Connect a team server to load tests'}</p></div></div>{tests.length === 0 ? <div className="empty-state compact"><div className="empty-icon"><Icon name="layers" /></div><p>No tests are registered for this organization yet.</p></div> : <div className="live-list-body">{tests.map((test) => <div className="live-list-row" key={test.id}><div><b>{test.name}</b><span>{test.manifestId} · {test.id}</span></div><button className="text-button" onClick={() => onRun(test)}>Run →</button></div>)}</div>}</section>; }
