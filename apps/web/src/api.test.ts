...
describe('web API client', () => {
  it('requires tenant config and sends tenant headers for live calls', async () => {
    ...
    const api = createApi({ baseUrl: 'http://server.test', organizationId: 'org-1' }, async (input, init) => { calls.push({ url: String(input), init }); return new Response(JSON.stringify({ tests: [] }), { status: 200, headers: { 'content-type': 'application/json' } }); });
    await api.listTests();
